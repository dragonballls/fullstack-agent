...
class VoiceActivationConfig:
    ...
    def from_env(cls) -> "VoiceActivationConfig":
        ...
        return cls(mode=mode if mode in {"open", "ptt"} else "open", wake_word=wake_word, wake_confidence=max(0.0, min(1.0, confidence)), post_wake_window_seconds=max(0.0, window), brain=brain, require_omniroute=require_omniroute, allow_claude=False)
