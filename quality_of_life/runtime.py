...
class JarvisRuntime:
    ...
    def _factory_from_spec(self, name: str) -> Callable[[], Any]:
        ...
        if name == "self_coding":
            ...
            # Direct main publication is intentionally not configurable at runtime.
            # Self-coding always creates a pending checkpoint; only an explicit
            # self_coding.approve action can promote it to main.
    ...
    def _register_actions(self) -> None:
        ...
        self.orchestrator.register(Action(Capability.REPO_WRITE, "self_coding.checkpoints", lambda: self._tool("self_coding").list_checkpoints()))
        self.orchestrator.register(Action(Capability.REPO_WRITE, "self_coding.approve", lambda checkpoint: self._tool("self_coding").approve_checkpoint(checkpoint)))
        self.orchestrator.register(Action(Capability.REPO_WRITE, "self_coding.undo", lambda checkpoint: self._tool("self_coding").undo_checkpoint(checkpoint)))
