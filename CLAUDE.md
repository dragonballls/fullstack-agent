...
- The toolbox also contains an optional `quality_of_life/` capability layer. Before using it, read `quality_of_life/README.md` and `quality_of_life/manifest.py`. Its capabilities are deny-by-default, and mutating operations must pass the shared capability policy and any required confirmation hook.
