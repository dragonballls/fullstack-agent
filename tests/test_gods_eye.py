...
class GodsEyeTests(unittest.TestCase):
    def test_geo_point_validates_bounds(self):
    ...
    def test_search_returns_structured_places_and_rejects_empty_queries(self):
        ...
        self.assertEqual(eye.search("Tokyo"), [place])
        self.assertEqual(geocoder.queries, ["Tokyo"])
        with self.assertRaises(ValueError):
            eye.search("   ")

    def test_locate_me_never_claims_a_location_without_permission(self):
        snapshot = LocationSnapshot(None, None, False, "permission-denied")
        eye = GodsEye(FakeGeocoder([]), FakeLocationProvider(snapshot))
        self.assertEqual(eye.locate_me(), snapshot)
        context = eye.context()
        self.assertFalse(context["location"]["permitted"])
        self.assertIsNone(context["location"]["point"])

    def test_route_is_deterministic_and_uses_valid_coordinates(self):
        ...
        self.assertEqual(route["origin"], {"latitude": 34.1, "longitude": -117.7})
        self.assertEqual(route["destination"], {"latitude": 34.0522, "longitude": -118.2437})
        self.assertEqual(route["provider"], "openstreetmap")
