...
def test_runtime_exposes_all_capability_tools_and_dispatches_gods_eye():
    ...
    assert {"computer", "screen", "browser", "clipboard", "windows", "background", "cloud_router", "gods_eye"} <= names
    places = runtime.dispatch(Capability.LOCATION_READ, "gods_eye.search", query="Tokyo")
    assert places[0].name == "Tokyo"


def test_runtime_preserves_deny_by_default():
...
def test_runtime_requires_confirmation_for_mutations():
    calls = []
    policy = CapabilityPolicy(frozenset({Capability.MOUSE_CONTROL}))
    runtime = JarvisRuntime(policy, factories={"computer": lambda: type("C", (), {"move": lambda self, x, y: calls.append((x, y))})()})
