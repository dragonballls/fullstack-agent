...
class FakeDispatcher:
    ...
    def run(self, capability, operation, *args, **kwargs):
...
class FakeRuntime:
    def __init__(self, allowed=None, require_confirmation=None):
        self.policy = CapabilityPolicy(
            allowed=frozenset(allowed or ()),
            require_confirmation=frozenset(require_confirmation or ()),
        )
...
class WorkflowOrchestrationTests(unittest.TestCase):
    def test_run_named_workflow_executes_steps_in_order(self):
        runtime = FakeRuntime(allowed={Capability.APP_READ, Capability.SYSTEM_DIAGNOSTICS})
    ...
    def test_missing_confirmation_does_not_execute_mutating_step(self):
        runtime = FakeRuntime(allowed={Capability.APP_LAUNCH}, require_confirmation={Capability.APP_LAUNCH})
